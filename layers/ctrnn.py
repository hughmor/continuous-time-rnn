...
"""
TODO: why did I write this whole class in such a weird way.. this should be improved
TODO: I think each neuron should be an object to allow for neuron specific params (function, bias, dynamics)
"""
...
class CTRNN:
    ...
    """
    def __init__(self, size: int = 1, n_in: int = 1, n_out: int = 1, **kwargs):
        # Network parameters
        self.size = size
        self.n_in = n_in
        self.n_out = n_out
        ...
        if kwargs.get('randomize weights', False):
            self.weight_matrix = np.random.random(size=(self.size, self.size))
            self.input_weights = np.random.random(size=(self.n_in, self.n_in))
        else:
            self.weight_matrix = np.array(kwargs.get('weight matrix', np.ones(shape=(self.size, self.size))))
            self.input_weights = np.array(kwargs.get('input weights', np.ones(shape=(self.n_in, self.n_in))))

        # Numerical integration params
        self.t_seconds = 0.0
        self._step_solver = None
        self.set_integration_mode(kwargs.get('integration mode', 'rk'))
        ...
        # Initialize vectors
        self._initial_state = np.array(kwargs.get('initial state', np.zeros(shape=self.size)))
        self.neuron_vector = self._initial_state
        self.bias_vector = np.array(kwargs.get('biases', np.zeros(shape=self.size)))
        #self.state_vector = self.activation_function(self.neuron_vector)
...
#region: initializing
    ...
    def set_integration_mode(self, mode:str):
        ...
        """
        if mode.lower() not in IntegrationSolvers.allowed_modes:
    ...
    def set_activation_mode(self, mode:str):
        """
        Setter for activation function type to use during evaluation.
    ...
    @staticmethod
    def _ds_dt(s:Union[float, np.ndarray], sb:Union[float, np.ndarray], tau:Union[float, np.ndarray], inpt:Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
    ...
    def ds_dt(self, t:float, s:Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        ...
        (This function exposes the correct signature to the integration method)
        
        :param t: Evaluation time.
        :param s: The state of the system.
        """
        return self._ds_dt(s, self.bias_vector, self.decay_constant, self._neuron_inputs)

    @property
    def state_vector(self):
        return self.activation_function(self.neuron_vector)

    def reset(self):
        """Resets the state of the network (sets neurons and time to zeros)."""
        self.neuron_vector = np.zeros_like(self.neuron_vector)
        self.t_seconds = 0.0
    ...
    def simulate(self, x:Union[list, np.ndarray], dt:float) -> np.ndarray:
        ...
        :param x: 2d input array with shape (number of inputs, number of samples).
        ...
        self.neuron_history = np.zeros(shape=(self.size, self.input_sequence.shape[1]))
        ...
        for i in range(self.input_sequence.shape[1]): # TODO: right now evaluation time is based on input length... change to time parameter as input
            self.advance(dt, dt, self.input_sequence[:,i])
            self.output_sequence[:,i] = self.state_vector[-self.n_out:]
            self.neuron_history[:,i] = self.neuron_vector[:]
    ...
    def advance(self, evolution_time:float, dt:float, inputs:Union[float, list, np.ndarray] = []) -> np.ndarray:
        ...
        while self.t_seconds < end_time:
            ...
            #self.state_vector = self.activation_function(self.neuron_vector) # Apply activation function (offset by bias)
            self.t_seconds += dt
    ...
    def plot_activation(self):
        ...
        f = plt.figure()
        x = np.linspace(0,1)
        plt.plot(x, self.activation_function(x))
        plt.xlabel('neuron state')
